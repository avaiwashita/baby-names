...
def compute_x(width, year_index):
    ...
    x = (SPACE - 1) + (year_index / total_years) * (width - (2 * SPACE))
    return x
...
def draw_fixed(canvas):
    ...
    canvas.create_line(SPACE - 1, SPACE - 1, width - SPACE - 1, SPACE - 1)
    canvas.create_line(SPACE - 1, height - SPACE - 1, width - SPACE - 1, height - SPACE - 1)

    for i in range(len(YEARS)):
        x = compute_x(width, i)
        canvas.create_line(x, 0, x, height - 1)
        canvas.create_text(x + TEXT_DX, height - SPACE - 1, text=YEARS[i], anchor=tkinter.NW)
...
def compute_rank(names, name, year):
    ...
    1000
    """
    if name in names and year in names[name] and names[name][year] < MAX_RANK:
...
def compute_y(names, name, height, year_index):
    ...
    >>> compute_y({'Abe': {1900: 1}}, 'Abe', 600, 0)
    19.0
    >>> compute_y({'Audrey': {1900: 1000}}, 'Ava', 600, 0)
    579.0
    """
    rank = compute_rank(names, name, YEARS[year_index])
    y = (SPACE - 1) + ((rank - 1) / (MAX_RANK - 1)) * (height - (2 * SPACE))
    return y
...
def draw_name(canvas, names, name, color):
    ...
    for i in range(len(YEARS)):
        ...
        rank = compute_rank(names, name, YEARS[i])
        if i != 0:
...
def draw_names(canvas, names, lookups):
    ...
    for i in range(len(lookups)):
        ...
        draw_name(canvas, names, name, color)
