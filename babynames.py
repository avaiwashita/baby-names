...
def add_name(names, year, rank, name):
    ...
    >>> add_name({'Jessica': {2000: 5}}, 2010, 7, 'Jessica')
    {'Jessica': {2000: 5, 2010: 7}}
    >>> add_name({'Christian': {2000: 22}}, 2000, 576, 'Christian')
    {'Christian': {2000: 22}}
    """
    if name not in names:
        names[name] = {}

    inner = names[name]  # access inner dict for year
    if year not in inner:
        inner[year] = rank
...
def add_file(names, filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            new_list = line.split(',')
            rank = int(new_list[0])
            boys_name = new_list[1]
            girls_name = new_list[2]
            add_name(names, year, rank, boys_name)
            add_name(names, year, rank, girls_name)
    return names
...
def search_names(names, target):
    ...
    (Code and tests TBD)
    >>> # Student Tests Here (keep this line)
    >>> search_names({'Emily': {2000: 1}, 'Emma': {2000: 11}}, 'em')
    ['Emily', 'Emma']
    >>> search_names({'Sasha': {2000: 54}, 'Sadie': {2000: 102}}, 'Sa')
    ['Sadie', 'Sasha']
    >>> search_names({'Tom': {2000: 248}, 'Anne': {2000: 341}}, 'ra')
    []
    """
    matches = []
